...
class AsyncTask:
 ...
 #####################################################################################################################################
        ...
        for ulPk in ulPk_list:
            ...
            result["ulPk"][1]["kor_time"] = kor_time
            result["ulPk"][2]["data"] = {}

#####################################################################################################################################
            for entity in entity_list:
                result["ulPk"][2]["data"][str(entity)] = 0
                for data in mon_list:
                    if ulPk == data.get("ulPk"):
                        if data.get("entity_id") == entity:
                            if("fan.xiaomi_miio_device" in data.get("entity_id")):
                                # result["ulPk"][2]["data"][entity] = data.get("attributes")
                                result["ulPk"][2]["data"]["fan_attribute"] = data.get("attributes")

                            elif ("cover.curtain" in data.get("entity_id")):
                                result["ulPk"][2]["data"]["cover_attribute"] = data.get("attributes")
                            else:
                                result["ulPk"][2]["data"][entity] = data.get("state")
            ...
            # ]
            # }
            # mongoUpdate(page_sanitized, "hs_sensor_series_2")
